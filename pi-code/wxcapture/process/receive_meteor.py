...
if PASS_INFO['satellite'] == 'METEOR-M 2':
    wxcutils.run_cmd('echo yes | /usr/bin/meteor_demod -B -o ' + WORKING_PATH +
                     WORKING_PATH +'.qpsk ' + AUDIO_PATH + FILENAME_BASE + '.wav')
if PASS_INFO['satellite'] == 'METEOR-M2 2':
    wxcutils.run_cmd('echo yes | /usr/bin/meteor_demod-old -B -o ' + WORKING_PATH +
                     FILENAME_BASE +'.qpsk ' + AUDIO_PATH + FILENAME_BASE + '.wav')
MY_LOGGER.debug('-' * 30)
...
if PASS_INFO['satellite'] == 'METEOR-M2 2':
    wxcutils.run_cmd('/usr/local/bin/medet_arm ' + WORKING_PATH + FILENAME_BASE + '.qpsk '
                     + WORKING_PATH + FILENAME_BASE + ' -S -cd -diff -int 80k')
MY_LOGGER.debug('-' * 30)

# Generate colour corrected .bmp
# active APIDs are at:
# http://happysat.nl/Meteor/html/Meteor_Status.html
wxcutils.run_cmd('/usr/local/bin/medet_arm ' + OUTPUT_PATH + FILENAME_BASE + '.dec ' +
