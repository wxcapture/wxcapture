#!/usr/bin/env python3
"""capture and process morse satellite pass
create audio, transcription plus pass web page"""
...
try:
    ...
    wxcutils.run_cmd('sox -t wav ' + AUDIO_PATH + FILENAME_BASE + '.wav -esigned-integer -b16 -r 22050 -t raw ' + AUDIO_PATH + FILENAME_BASE + '.raw')

    # multimon-ng -t raw -a MORSE_CW morse_sample.raw /dev/stdin >> op.txt
    MY_LOGGER.debug('Parse text with multimon-ng')
    wxcutils.run_cmd('multimon-ng -t raw -a MORSE_CW ' + AUDIO_PATH + FILENAME_BASE + '.raw > ' + OUTPUT_PATH + FILENAME_BASE + '-decode.txt')

    # read in the file
    DECODE_TEXT = wxcutils.load_file(OUTPUT_PATH, FILENAME_BASE + '-decode.txt')
    ...
    with open(OUTPUT_PATH + FILENAME_BASE + '.html', 'w') as html:
        ...
        html.write('<title>Satellite Pass Audio and Decode</title></head>')
        ...
        html.write('<h2>Decoded text from audio</h2>')
        html.write('<p>' + DECODE_TEXT + '</p>')
